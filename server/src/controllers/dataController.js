...
const querySchema = zod_1.z.object({
    ...
    name: zod_1.z
        ...
        zod_1.z.array(zod_1.z.string().trim().min(1)),
    ])
        .optional(),
    from: zod_1.z.string().trim().optional(),
    to: zod_1.z.string().trim().optional(),
    interval: zod_1.z.enum(VALID_INTERVALS).optional(),
});
// Helper to get PostgreSQL interval expression
const getIntervalTruncation = (interval) => {
    switch (interval) {
        case "1m":
...
_a = DataController;
DataController.getByDevice = (0, asyncHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (interval) {
      ...
      `;
        const { rows: countRows } = await db_1.pool.query(countQuery, values);
      ...
      `;
        const { rows } = await db_1.pool.query(aggregateQuery, aggregateValues);
    ...
    const { rows: countRows } = await db_1.pool.query(`SELECT sp.name, COUNT(*)::int AS cnt
       FROM sensor_parameter sp
       JOIN sensor_device sd ON sp.device_id = sd.id
       WHERE ${where}
       GROUP BY sp.name`, values);
    // Build the per-parameter counts map and overall total
    const parameterCounts = {};
    let total = 0;
    for (const r of countRows) {
        parameterCounts[r.name] = r.cnt;
        total += r.cnt;
    }
    values.push(limit, (page - 1) * limit);
    const { rows } = await db_1.pool.query(`SELECT sp.id,
              sp.name,
              sp.value,
              sp.unit,
              sp.status,
